...
**写操作不设预留名额**（2026-09-12 决策，不必重开讨论）。曾考虑把闸门拆成
读/写两个配额，理由是「确认动作不该因读查询挤满而被拒」。否掉的理由：
写操作本来就要二次确认，429 发生在确认**之前**，用户重来一次即可，
不产生任何模糊状态；而拆配额会引入一个新的故障模式（预留名额被占满、
读的可用额度变少），换来的却是一个 PoC 里几乎不会发生的场景 ——
演示流量里绝大多数是读。
"""
