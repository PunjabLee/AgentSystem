...
| 总工作量 | 62.8 人天（v1.8 重估） |
| 团队规模 | 1 人（全串行） |
| 日历周期 | 15.7 周，含缓冲约 19–20 周 |
...
|---|---|---|---|
| **P0 前置验证** | — | **4.3** | 全新增 |
| **P1 地基** | 4.0 | **6.0** | 审计基座含变更前后值与两段式写入、`REVOKE UPDATE/DELETE` 防篡改；CI 收敛为不含 LLM 的宪法机械检查（0.75）；`make backup` + **恢复演练**（0.35）；宪法修订（第十条不可信输入 + 版本同步） |
...
| **P5 评测 + 交付物** | 7.0 | **17.7** | RAG 独立 50 条评测集 + 注入 8 条 + 边界扩充（2.0）；失败类型归因（意图/槽位/工具格式/业务逻辑）；**3 档 × 3 次运行**；LLM-as-judge 双判卷 + 人机一致率（1.5）；**参数化 TCO 模型**（0.75）；真实系统对接工作量估算 + 场景价值排序（1.0）；前端剩余四组件（2.0）；业务方真人试用（0.3） |
| **合计** | **36.0** | **62.8** | |
...
| 62.8 人天 ÷ **4 有效天/周**（现实口径） | **15.7 周** |
| + 25% 缓冲（匹配未测组合数量，集中持有不铺进各阶段） | **约 19–20 周** |
