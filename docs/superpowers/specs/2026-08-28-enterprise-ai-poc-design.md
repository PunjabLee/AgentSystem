...
| 团队规模 | 1 人（全串行） |
| 日历周期 | 7.2 周，含缓冲约 8 周 |
| 关联文档 | [项目宪法](../../CONSTITUTION.md) · [技术栈版本矩阵](../../TECH-STACK-VERSIONS.md) |
...
│  档位切换 M0-M3 ▾ │ 引用来源卡片 │ 二次确认弹窗 │ 审计追溯视图        │
└────────────────────────────┬──────────────────────────────────────┘
                             │  SSE 流式 / REST
┌────────────────────────────▼──────────────────────────────────────┐
...
| **Lobe Chat** | 模型档位切换的 UI 范式，对应 M0–M3 切换器 |
...
| 档位切换器 | M0–M3 切换，显示当前档位与延迟 | Select + Tag |
...
### 里程碑二：完整能力与交付物（P4–P5，16 人天）

| 阶段 | 交付 | 验收信号 | 人天 |
|---|---|---|---|
| **P4 Workflow + RPA** | Dify 十二项场景、**`NotifierPort` + 企微/钉钉/飞书/邮件四适配器**、模拟遗留 ERP Web UI、Playwright RPA、API 超时自动降级、影刀流程设计文档 | 「API 挂掉 → RPA 兜底 → 回填结果」可演示 | 9 |
| **P5 评测 + 交付物** | ≥40 条测试集、M1/M2/M3 三档评测、10 并发压测、向量库切换验证（pgvector → Qdrant 重跑评测）、架构文档、WBS、API 文档、落地手册、部署运维、资源计划报告、《向量数据纳管服务设计》、**《AutoDL 部署配置参数设置指南》** | 报告出数，给出选型建议 | 7 |

每阶段末含 0.5 人天 **converge 收敛检查**（§2.4），以本文档与 `CONSTITUTION.md` 为基准核对实现，产出偏差清单并记录处置。

### 工期换算

**总工作量 36.0 人天，团队 1 人，全串行。**
...
工作量自 v1.2 的 31.5 增至 36.0，构成：v1.4 引入的 constitution（0.5）与五次 converge 检查（2.5）此前未计入；通知渠道由待定改为四渠道全实现（+1.5）；新增《AutoDL 配置参数设置指南》（+1.0）；模型档位五并为四、评测少一档（−1.0）。

**单人串行的特有风险**：无代码评审的第二双眼睛，宪法核对与 converge 检查的重要性相应上升——它们是本项目仅有的系统性纠偏机制。
