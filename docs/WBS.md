...
**出口判据**：`make chat TIER=M1` 与 `TIER=M3` 均能回话并返回结构化 `tool_calls`；CI 绿；`make backup` 已演练过一次恢复。
...
| P1.2.2 | **`extra_body` 段：thinking 模式开关按档位注入**【评审】<br>三档形状各不相同：Ollama 顶层 `think`、vLLM `chat_template_kwargs.enable_thinking`、百炼 `extra_body.enable_thinking` | 0.4 | P1.2.1 |
...
| P5.3 | 三档 × 3 次运行，固定温度与种子 | 1.0 | 【评审】否则运行间方差与档位差异混淆 |
...
| D2 | **阿里云百炼真实 API Key** | 用户 | P1 开工前 | M3 档、核心对照组的一半 | 🔴 **`.env` 中为 5 字符占位值** |
| D3 | 百炼是否上架 qwen3.8-27b | 待 D2 后核实 | P1 内 | M2 vs M3「同权重同规格」前提 | ⏸ 依赖 D2 |
...
**D1 与 D2 是硬阻塞**：没有它们，P1 的出口判据（`make chat TIER=M1` / `TIER=M3` 均能回话）无法达成，而 P1 是所有后续阶段的地基。
