...
> **本 WBS 不用于重估工期。** 设计文档 v2.1 的 62.8 人天维持不变，此处人天仅作任务排序与依赖判断之用。
>
...
> ## ✅ P1 完成情况（2026-09-03 · 分支 `feat/p1-foundation`）
>
> **三条出口判据全部 PASS**，`make p1-exit` 可复现判定。
>
> | 判据 | 结果 | 证据 |
> |---|---|---|
> | 1 三档 tool_calls | PASS | M0/M3/M4 逐项判定：`finish_reason=="tool_calls"` ∧ 命中 stub ∧ arguments 可解析 |
> | 2 CI 绿 | PASS | 69 项；含点名的回滚路径与原子消费两个单测 |
> | 3 恢复演练 | PASS | 独立容器恢复，行数一致 **且三层防护复现** |
>
> 25 项任务全部落地。过程中修正的三处偏差：
>
> 1. **P1.3.5 只建了三个角色**（应为四个）。查下去发现 `dify` 库锁了 PUBLIC 而
>    `agentsystem` 没锁 —— 方向做反了。已补 `dify_owner` 与库级隔离。
> 2. **`audit_log` 漏了 `client_trace_id`**（§2.5.3 要求）。补列被事件触发器拦下，
>    因此新增 `make migrate-audit` 的解锁流程。
> 3. **`.gitignore` 的裸 `models/` 吞掉了 ORM 模型目录**，两张表的模型从未入库，
>    且因 ruff 遵循 gitignore 而一直躲过 lint。
>
> 🔴 **遗留一项待决策**：Dify 仍以 `postgres`（超级用户）连库，超级用户绕过一切
> ACL，P1.3.4 的三层防护对它无效。切换脚本 `scripts/05` 已备好但未执行 ——
> 需停机、转移 145+13 张表属主、改 Dify 的 `.env`。见
> [部署运维说明 §3](DEPLOYMENT.md)。
...
| P2.2.2 | 生成器 + **固定随机种子**；产品目录按「少品种多批次」约束（否则跨缸澄清无数据可演）【评审】<br>🔴 **必须按拓扑序生成**（主数据 → 订单头 → 订单行 → 计划）—— 不建外键的补偿手段之一，见[设计文档 §4.0](superpowers/specs/2026-08-28-enterprise-ai-poc-design.md) | 0.6 | P2.2.1 |
| P2.2.3 | **生成后断言校验**：延误订单、库存不足、待检批次、跨 BU 同名产品等关键形态必须存在<br>【P0】P0-2 造数据时四维完全相关导致过滤查询返回 0 行，此项非可选<br>🔴 **另须断言孤儿行数为 0**（`sales_order_line.order_no`、`production_plan.related_order_line` 各自能对上）—— 不建外键后数据库不再拦脏数据，这是 §4.0 决策成立的**前提**而非建议 | 0.4 | P2.2.2 |
...
| P2.3.4 | 五类查询端点 + 创建订单写端点（✅ **契约已在 [P2 详细设计 §3 / §3.2](design/P2-DETAILED-DESIGN.md) 定稿**）<br>🔴 不建外键的两项补偿（[§4.0](superpowers/specs/2026-08-28-enterprise-ai-poc-design.md)，非可选）：<br>① 只读端点的 JOIN **一律 `INNER JOIN`** —— 孤儿行自然不进结果<br>② 写端点插入订单行前**校验 `order_no` 存在** | 0.8 | P2.3.2 |
...
|---|---|---|---|---|---|
| **D1** | **AutoDL 实例 + SSH 凭据** | 用户 | 🔴 **P2 结束（约第 6 周）** | P0-7、M1/M2 全部评测、TCO 的自建曲线 | 🟡 实例已开通（无卡模式），**SSH 凭据未提供** |
...
**D1 的死线已重设为 P2 结束（约第 6 周），到点未到位自动触发砍范围**，见[设计文档 §15.3](superpowers/specs/2026-08-28-enterprise-ai-poc-design.md)。

原死线「P1 开工前」在出口判据改为 M0/M3/M4 后**不再触发任何动作**——D1 可一路静默滑到 P5 才被发现缺失，届时 M1/M2 无从补做，「自建 vs 采购」这个核心结论直接作废。新死线选 P2 结束是因为**失败后还有回旋余地**：P3 尚未开工，砍掉自建档对 P3/P4 无影响。

原文：原文：（`make chat TIER=M1` / `TIER=M2` 能回话并返回结构化 `tool_calls`）无法达成，而 P1 是所有后续阶段的地基。D2 已由用户确认具备，注入 `.env` 即可。
