...
> ### ⚠️ 本节的卡型与量化结论已被推翻，以 [AUTODL-SPEC v1.2](AUTODL-SPEC.md) 为准
>
> 下文按 **4090 24G + INT4** 推演。该前提在用户确认可提供 **5090 32G** 后失效，且当时的 KV/token 测算（256 KiB）也因未查 `layer_types` 而高估 4 倍——Qwen3.8 是混合注意力架构，48 层线性 + 16 层全注意力，真实值 **64 KiB**。
>
> **现行结论**：M1 = 5090 32G + `unsloth/Qwen3.8-27B-NVFP4`（21.8 GiB，↓180 万），`--kv-cache-dtype fp8` 为**可选**而非必须。
>
> 本节保留是为记录推理过程与「型号一直是对的」这一发现，**不可作为采购或部署依据**。
